...
import { TranslateRestourant } from './../localisation_restaurant/translate.restaurant/schemas/translate.schema';
import {
  DefinitionRestourant,
  RestourantTranslateService,
} from '../localisation_restaurant/translate.restaurant';
...
export class FoodService {
  constructor(
    ...
    private readonly languageRestourantModel: Model<LanguageRestourant>,
  ...
  async searchFood(payload: SearchFoodInterface): Promise<Food[]> {
    ...
    const data = await this.getFoodList(
    ...
    for (const food of data) {
      if (
        (food.name
          ...
          food.restourant_id.toString() == payload.restaurant_id) ||
        (food.description
          .toString()
          .toLocaleLowerCase()
          .includes(payload.name.toLocaleLowerCase()) &&
  ...
  async getFoodList(
    ...
    const data = await this.foodModel
      ...
      .select(
        'name description image_urls price food_status restourant_id preparing_time',
      )
      .exec();
    ...
    for (let x of data) {
      ...
      const translated_name =
        await this.service.getSingleRestourantTranslate(name_request);
      const translated_description =
        await this.service.getSingleRestourantTranslate(desription_request);
      result.push({
        id: x._id,
    ...
    if (payload.name) {
      ...
      for (let languageCode of name_kays_array) {
        await this.#_checkLanguage(
          languageCode,
          translatefindByID.restourant_id.toString(),
        );
      }
      ...
      await this.definitionRestourantModel.deleteMany({
