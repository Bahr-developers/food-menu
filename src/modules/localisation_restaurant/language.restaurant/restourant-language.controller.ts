...
import {
  CreateRestourantLanguageDto,
  UpdateLanguageRestourantDto,
} from './dtos';
...
export class RestourantLanguageController {
  constructor(
    private readonly restourantLanguageService: RestourantLanguageService,
  ) {}

  @Get(':restaurantId')
  async findAll(
    @Param('restaurantId') id: string,
  ): Promise<LanguageRestourant[]> {
  ...
  async create(
    @Body() payload: CreateRestourantLanguageDto,
  ...
  async updateLanguage(
    @Param('id') id: string,
    @Body() payload: UpdateLanguageRestourantDto,
    @UploadedFile() image: any,
  ) {
    await this.restourantLanguageService.updateLanguage({
      ...payload,
      id,
      image,
    });
