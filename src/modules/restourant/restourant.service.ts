...
import {
  TranslateRestourant,
  TranslateRestourantSchema,
} from './../localisation_restaurant/translate.restaurant/schemas/translate.schema';
...
export class RestourantService {
    ...
    for (let x of data) {
      ...
      if (x.description?.length) {
        ...
        const translated_description =
          await this.service.getSingleTranslate(description_request);
        value = translated_description.value;
      }

      const translated_name =
        await this.service.getSingleTranslate(name_request);

      if (x.socials.length) {
        for (const item of x.socials) {
          socials.push({
            link: item.link,
            socials: await this.socialModel.findOne({ _id: item.social_id }),
          });
        }
      } else {
        socials = [];
  ...
  async addOneSocial(payload: AddOneSocialInterface): Promise<void> {
    ...
    await this.restourantModel.findByIdAndUpdate(payload.restaurant_id, {
      $push: {
        socials: {
          social_id: payload.social.social_id,
          link: payload.social.link,
        },
      },
    });
  }
  ...
  async deleteOneSocial(payload: DeleteOneSocialInterface): Promise<void> {
    ...
    const foundedRestourant = await this.restourantModel.findById(
